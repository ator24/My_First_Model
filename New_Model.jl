...
# ╔═╡ 325230bb-ee8c-435c-87c8-dcaa6e076698
heaviside(x) = x < 0 ? 0.0 : 1.0

# ╔═╡ 4e29533b-9963-4465-91f0-e5acfde8dbae
function local_interactions2!(du, u, p, t)
	qi, qd, qv, Si, T = u

	Ni, ν, w = p

	#masses of ice crystals and droplets in the monodisperse distribution
    mi = qi/Ni
    md = qd/(Ni*ν)
	...
	if qi > 0
        ri = ai*mi^b
        dqi = 4*pi*Ni*G_ice*ri*si
        if qd > 0
            rd = ad*md^b
			ξ1 = (4*pi*ν*Ni*rd)^1/2
            dqi = dqi + 4*pi*G_ice*Ni*ri^2 * ξ1*sid*heaviside(qd)
        end
    else
        dqi = 0
    end
    ...
    if qd > 0 
        rd = ad*md^b
        dqd = 4*pi*ν*Ni*G_drop*rd*sd*heaviside(qd)
        if qi > 0 
            ri = ai*mi^b
            ξ1 = (4*pi*ν*Ni*rd)^1/2
            dqd = dqd -  4*pi*G_ice*ri^2 * ξ1*sid
        end
    else
        dqd = 0
    end
    ...
    du .= [dqi, dqd, dqv, dSi, dT]
end

# ╔═╡ faa00955-fc1d-41ee-9aec-8a23e1b5ab75
tspan2 = (0.0, 5000.0)

# ╔═╡ ffb67a6f-528c-4d19-8d7d-4b2537c1cb13
params2 = (1000, 40, -0.5, 0.0)

# ╔═╡ a39fd3e3-1779-46a0-88a6-31de699e1cfd
u02 = [1.6*10^-5, 1.0*10^-4, 1.65*10^-3, 1.5, 258.15]

# ╔═╡ 9054ff21-15c1-4f36-a64a-ed98bf586b0b
prob2 = ODEProblem(local_interactions2!, u02, tspan2, params2)

# ╔═╡ 2a09fea4-86ff-4560-8e5f-805a7e9d673c
sol2 = solve(prob2, TRBDF2(), saveat=0.1)

# ╔═╡ 5616bb90-f218-463b-9fe1-76155f9c4e38
qi2 = [u[1] for u in sol2.u]

# ╔═╡ 8f91400b-be70-41c2-a35f-b53d6ee454fb
qd2 = [u[2] for u in sol2.u];

# ╔═╡ b4c5ed52-e0fa-480e-a25f-dba2648fff71
qv2 = [u[3] for u in sol2.u];

# ╔═╡ 5d2e4f3a-dedb-4d37-b794-7329f64015bc
Si2 = [u[4] for u in sol2.u];

# ╔═╡ 1577bd0e-0ed9-4c9b-bbc6-a41a6ae1f3bd
T2 = [u[5] for u in sol2.u];

# ╔═╡ a604197d-0da4-4a2c-beb4-2e83bb359ee4
plot(sol2.t, qi2)

# ╔═╡ fa378ab8-bdbe-47c1-8343-ed01be1cff67
plot(sol2.t, qd2)

# ╔═╡ 40272cd7-64e8-4867-a773-d6ece7597498
plot(sol2.t, Si2)

# ╔═╡ 37587327-8020-488c-a3be-2cb738bc8143
plot(sol2.t, qv2)

# ╔═╡ b0bc678e-cde1-4c7d-970b-dd8ae3012a47
plot(sol2.t, T2)
...
manifest_format = "2.0"
project_hash = "4e03af9417757e2aadafa38ade6826eaed86c95d"
