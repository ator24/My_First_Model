...
# ╔═╡ ed6477fa-065c-11f0-107b-078f3e67f55e
using DifferentialEquations, Plots, Statistics

# ╔═╡ 9aa8c5ca-2cd0-4781-9b16-c425b776c7cc
const Rv = 461.5    #Specific gas constant for water vapor (J/kg K)

# ╔═╡ 9c3984a0-f802-4774-afe4-3b2a6d138966
const Ra = 287.05         # Gas constant for dry air [J/(kg·K)]

# ╔═╡ 4324bf7d-809c-4115-9e62-dd1708d29195
const L_iv = 2.83e6   #Latent heat of sublibation for ice (J/kg)

# ╔═╡ b1583539-ba0f-4c9c-9c7b-c6d83f96f2cf
const L_lv = 2.26e6  # Latent heat of vaporization for water [J/kg]

# ╔═╡ d4c130b6-24a9-4d7a-acd8-09b193ffcf71
const rho_i = 917.0  #Density of ice (kg/m^3)

# ╔═╡ 566ee42b-78f7-4bb9-b505-fc444c7e2773
const rho_l = 998.0  #Density of liquid water (kg/m^3)

# ╔═╡ a890846d-85b9-4a61-9c08-444352070b21
const Dv = 2.11e-5   #Diffusivity of water vapor in air (m^2/s)

# ╔═╡ 2c5fc8a2-eb38-476d-931b-fa9c4f898f9d
const cs = 2.2e-2    #Thermal conductivity of air (W/m*K)

# ╔═╡ 2a79cfdf-1096-45d2-a579-0314784abae3
const g = 9.81      #gravity [m/s^2]

# ╔═╡ c2fafb8b-0566-44a7-87cf-45d5b3c153a5
const cp_a = 1005.0   # Specific heat capacity of dry air [J/(kg·K)]

# ╔═╡ c21b3777-521d-4ec2-96fb-00209e86917d
const cp_v = 1996.0   #specific heat capacity of water vapor [J/(kg*K)]

# ╔═╡ 219c3d5d-5a62-4098-9df5-78789ff6f171
const ad = (3/(4*pi*rho_l))^(1/3)   #term that appears in the liquid droplet mass

# ╔═╡ e2fc6d91-fe41-4fb2-8f42-df71fa0a6c84
const ai = (3/(4*pi*rho_l))^(1/3) #term that appears in the ice crystals mass

# ╔═╡ 71c48d27-25eb-4b81-84b2-4bcfbfcfc00d
const b = 1/3       #define the exponent in a nicer way

# ╔═╡ e4b816dc-e47b-4dfc-b6fa-ead5701b6608
const ε = Ra/Rv     #ratio of the gas constants

# ╔═╡ 3cde674d-254e-4d6c-80cb-2e4b169fa9d1
begin
	function e_sat_liquid(T)
	    return 611*exp(L_lv/Rv * (1/273.15 - 1/T))
	end
	
	function e_sat_ice(T) 
	    return 611*exp(L_iv/Rv * (1/273.15 - 1/T))
	end
	
	function G_h_ice(T)
	    return 4*pi/( (L_iv/(Rv*T) - 1)*L_iv/(cs*T) + Rv*T/(Dv*e_sat_ice(T) ) )
	end   
	
	function G_h_liquid(T)
	    return 4*pi/( (L_lv/(Rv*T) - 1)*L_lv/(cs*T) + Rv*T/(Dv*e_sat_liquid(T) ) )
	end  
end

# ╔═╡ e7dcc8cf-b79a-4a66-9d9e-d2d45259db50
heaviside(x) = x < 0 ? 0.0 : 1.0

# ╔═╡ 3b94eed9-4d62-4981-8dcc-7f37e5dd2afb
function local_interavtions_N!(du, u, p, t)
	qi, Ni, qd, ν, qv, Si, T = u

	w, σ = p



	#Howell factors for both ice and droplets
    G_ice = G_h_ice(T)
    G_drop = G_h_liquid(T)

    #Saturating ratio of liquid droplets
    S = Si*e_sat_ice(T)/e_sat_liquid(T)

	#supersaturations
	si = Si - 1
	sd = S - 1
	sid = (e_sat_liquid(T) - e_sat_ice(T))/e_sat_ice(T)

    #Gas constant and heat capacity of moist air
    R_bar = Ra*(1 + ((1-ε)/ε)*qv/(1+qv))
    cp_bar = cp_a*(1 + (cp_v/cp_a - 1)*(qv/(1+qv))) 

	#Growth constants 
	A = 4*pi*G_ice*(3/(4*pi*rho_i))^(1/3)
	B = 4*pi*G_drop*(3/(4*pi*rho_l))^(1/3)
	C = (4*pi)^(3/2) *G_ice*(3/(4*pi*rho_i))^(2/3) * (3/(4*pi*rho_l))^(1/6)


	if qi > 0 && Ni > 0
		dqi = A* Ni^(2/3) * qi^(1/3) * si
		if ν > 0 && qd > 0
			dqi = dqi + C*Ni^(2/3) * qi^(2/3) * qd^(1/6) * ν^(1/6) * sid
		end
	else
		dqi = 0
	end
	...
	if Ni > 0 && qi > 0 
		dNi = A*Ni^(5/3) * qi^(-2/3) *si*heaviside(-si)
	else
		dNi = 0
	end

	if qd > 0 && ν > 0 && Ni > 0
		dqd = B * Ni^(2/3) * ν^(2/3) * qd^(1/3) * sd 

		if qi > 0
			dqd = dqd - C*Ni^(2/3) * qi^(2/3) * qd^(1/6) * ν^(1/6) * sid
		end
	else
		dqd = 0
	end

	if qd > 0 && Ni > 0 && ν > 0 && qi > 0
		dν = B * qd^(-2/3) * Ni^(2/3) * ν^(5/3) * sd*heaviside(-sd) + C*qi^(2/3) * Ni^(-1/3) * qd^(1/6) * ν^(4/3) * sid - A * Ni^(2/3) * qi^(-2/3) * ν * si*heaviside(-si)
	else
		dν = 0
	end
	...
	#evolution of vapor mixing ratio
    dqv = -(dqi + dqd)

    #evolution of temperature
    dT = -qv*w/cp_bar + (1/(cp_bar*(1+qv)))*(L_lv*dqd + L_iv*dqi)

	#evolution for the ice saturating ratio
    dSi = (L_iv/(cp_v*Rv*T^2) - 1/(Ra*T))*g*w*Si + Si*dqv/qv

	du .= [dqi, dNi, dqd, dν, dqv, dSi, dT]
end

# ╔═╡ 558b416c-776c-48fd-85a8-0d39e35d075e
fa = 30

# ╔═╡ d3068a72-36c7-48fb-a350-1a141cd2b13b
tspan2 = (0.0, 5000.0)

# ╔═╡ f17c937c-c3f7-472c-b765-a7f99e3f2ba6
params2 = (0.5, 0.0)

# ╔═╡ 56d3869b-beda-42f0-b9ed-a27bcc0a6ecb
u02 = [1.6*10^-5, 1000, 5*10^-5, 100, 1.65*10^-3, 1.01, 258.15]

# ╔═╡ 3be71c63-9e27-40c5-a33e-66f67344789b
prob2 = ODEProblem(local_interavtions_N!, u02, tspan2, params2)

# ╔═╡ 14ed5010-d0a2-42ce-b281-c7c0ac8f7d06
sol2 = solve(prob2, TRBDF2(), saveat=0.1)

# ╔═╡ 8e85b39d-85cc-4df8-afe3-845b93be3377
qi2 = [u[1] for u in sol2.u];

# ╔═╡ 12f122aa-8165-4509-85d5-c304a9543d0b
Ni = [u[2] for u in sol2.u];

# ╔═╡ 0918678b-f563-474c-9520-b0724d4d6a49
qd2 = [u[3] for u in sol2.u];

# ╔═╡ e8319110-0d47-4478-8afd-a2959607714d
ν2 = [u[4] for u in sol2.u];

# ╔═╡ ff84e8e5-74c3-4b03-b76e-c9dd77aee840
qv2 = [u[5] for u in sol2.u];

# ╔═╡ 5a77f238-dd09-4954-a469-4ac1b06030a7
Si2 = [u[6] for u in sol2.u];

# ╔═╡ 4ad58985-869b-4406-8f55-9d63778bc309
T2 = [u[7] for u in sol2.u];

# ╔═╡ 52b6646c-0ad4-4070-82d6-fdb9b39068bc
plot(sol2.t, qi2)

# ╔═╡ 4054905e-544e-4a3b-b4f1-369b13943c5a
plot(sol2.t, qd2)

# ╔═╡ 9ceaa8b3-a2e9-4df5-ba85-7ffb75bea3d7
plot(sol2.t, ν2)

# ╔═╡ dd7e144f-47c7-4d49-beab-75eb1dc1fe1f
plot(sol2.t, Ni)

# ╔═╡ 899683ab-3273-433b-a820-f810d4b899da
plot(sol2.t, qv2)

# ╔═╡ eed80716-75fa-45a3-a163-cf0c1a8e2657
plot(sol2.t, Si2)

# ╔═╡ f69cff33-628e-4791-9fd9-954f31c1ceac
...
manifest_format = "2.0"
project_hash = "5498aaf80ca358b45bf1884b42e9cc2a3cf5f8f4"
